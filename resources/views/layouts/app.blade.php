...
<body class="font-sans antialiased bg-gray-50">
    @auth
        <div class="min-h-screen">
            <!-- Sidebar -->
            <div id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 transition-transform duration-300 ease-in-out transform -translate-x-full shadow-xl sidebar-toggle sidebar-gradient md:translate-x-0">
                <!-- Sidebar Header -->
    ...
    <div class="space-y-2">
        ...
        @permission('roles.view') {{-- Added for consistency --}}
            <a href="{{ route('roles.index') }}" class="group flex items-center px-4 py-3 text-sm font-medium rounded-lg {{ request()->routeIs('roles.*') ? 'bg-white bg-opacity-20 text-white' : 'text-blue-100 hover:bg-white hover:bg-opacity-10 hover:text-white' }} transition-colors duration-200">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    {{-- Icon for Roles (shield) --}}
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 20.944L12 23l9-2.056a12.02 12.02 0 00-2.382-9.025z" />
                </svg>
                Roles
            </a>
        @endpermission

        @permission('providers.view') {{-- Added for consistency --}}
            <a href="{{ route('providers.index') }}" class="group flex items-center px-4 py-3 text-sm font-medium rounded-lg {{ request()->routeIs('providers.*') ? 'bg-white bg-opacity-20 text-white' : 'text-blue-100 hover:bg-white hover:bg-opacity-10 hover:text-white' }} transition-colors duration-200">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    {{-- Icon for Providers (truck) --}}
                    <path d="M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z"></path>
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10l2 2h8l2-2z"></path>
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h2a1 1 0 001-1V7h2v8a1 1 0 001 1h2a1 1 0 001-1V5a1 1 0 00-1-1H9a1 1 0 00-1 1v1"></path>
                </svg>
                Providers
            </a>
        @endpermission
        ...
        <div class="pt-4 mt-4 border-t border-blue-400 border-opacity-30">
            <p class="px-4 text-xs font-semibold tracking-wider text-blue-200 uppercase">Account</p>
                ...
                <div class="absolute bottom-0 w-full p-4">
                    <div class="p-3 bg-white rounded-lg bg-opacity-10">
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="mt-3" id="logout-form">
                            @csrf
                            <button type="button" onclick="confirmLogout()" class="flex items-center justify-center w-full px-3 py-2 text-sm font-medium text-blue-100 transition-colors duration-200 bg-white rounded-md bg-opacity-10 hover:bg-opacity-20">
            ...
            <!-- Main Content -->
            <div id="main-content" class="transition-all duration-300 ease-in-out main-content">
                <!-- Top Navigation Bar -->
                <nav class="border-b border-gray-200 shadow-sm navbar-gradient">
                    <div class="px-6 py-4">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                <!-- Mobile menu button -->
                                <button id="sidebar-toggle" type="button" class="inline-flex items-center justify-center p-2 text-gray-500 rounded-md md:hidden hover:text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500">
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="relative hidden md:block">
                                    ...
                                    <input type="text" placeholder="Quick search..." class="block w-64 py-2 pl-10 pr-3 text-sm leading-5 placeholder-gray-500 bg-white border border-gray-200 rounded-lg focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Page Content -->
                <main class="min-h-screen p-6 bg-gradient-to-br from-gray-50 via-blue-50/30 to-purple-50/30">
                    <!-- Modern Flash Messages -->
                    @if (session('message'))
                        <div class="px-6 py-4 mb-6 text-green-700 border border-green-200 shadow-lg bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl backdrop-blur-sm" role="alert">
                            <div class="flex items-center">
                    ...
                    @if (session('error'))
                        <div class="px-6 py-4 mb-6 text-red-700 border border-red-200 shadow-lg bg-gradient-to-r from-red-50 to-pink-50 rounded-2xl backdrop-blur-sm" role="alert">
                            <div class="flex items-center">
                                <div class="flex items-center justify-center w-8 h-8 mr-3 bg-red-100 rounded-full">
                ...
                <!-- Modern Footer -->
                <footer class="border-t border-gray-200 shadow-lg bg-gradient-to-r from-white via-gray-50 to-white">
                    <div class="px-6 py-6">
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <div class="flex items-center space-x-6 text-gray-500">
                                ...
                                <div class="items-center hidden space-x-4 md:flex">
                                    <a href="#" class="flex items-center space-x-1 transition-colors duration-200 hover:text-blue-600">
        ...
        <div id="sidebar-overlay" class="fixed inset-0 z-40 transition-opacity duration-300 ease-linear transform bg-gray-600 bg-opacity-75 opacity-0 pointer-events-none md:hidden"></div>
    ...
    @else
        ...
        <div class="flex items-center justify-center min-h-screen">
            <div class="text-center">
                ...
                <a href="{{ route('login') }}" class="inline-block px-4 py-2 mt-4 text-white bg-blue-600 rounded-md hover:bg-blue-700">Login</a>
            </div>
        </div>
    @endauth
    ...
    <script>
        ...
        window.showAlert = function(type, title, text, options = {}) {
            Notiflix.Report[type](title, text, 'OK', options);
        };
        ...
        window.showWarning = function(title, text = '') {
            Notiflix.Notify.warning(title || text);
        };

        window.showInfo = function(title, text = '') {
            Notiflix.Notify.info(title || text);
        };

        // Toast notifications
        window.showToast = function(type, title) {
            Notiflix.Notify[type](title);
        };
        ...
        @if (session('error'))
            showToast('failure', '{{ session('error') }}');
        @endif
